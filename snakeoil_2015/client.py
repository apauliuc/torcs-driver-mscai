...
if __name__ == "__main__":
    ...
    if C.stage > 0:
        f = codecs.open('csv/race-{}.csv'.format(C.trackname), 'a', encoding='UTF-8')
    ...
    for step in xrange(C.maxSteps, 0, -1):
        ...
        if C.stage > 0:
            ...
            row.append(C.S.d['speedY'])
            row.append(C.S.d['angle'])
            row.append(C.S.d['gear'])
            row.append(C.S.d['rpm'])
            for value in C.S.d['wheelSpinVel']:
                row.append(value)
            for value in C.S.d['track']:
                row.append(value)

            # response
            row.append(C.R.d['gear'])
